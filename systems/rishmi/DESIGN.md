---
name: Official
description: Israeli civic services with a clean redesign, Hebrew-first. White surface, deep navy ink, Israeli-flag blue accent. Trustworthy, accessible, neutral.
colors:
  primary: "#0A2540"
  on-primary: "#FFFFFF"
  surface: "#FFFFFF"
...
Hebrew-first. Heebo carries every size — display, headline, body — paired with Inter as a Latin fallback for English-only surfaces. The single-family choice is intentional: a citizen's form should not require two fonts to communicate.

## Colors

Three working colors plus a cool surface-container.

- **Primary (#0A2540)**: deep navy, the trust color. Body text, inverted CTAs, headers.
- **On primary (#FFFFFF)**: pure white sitting on primary.
- **Surface (#FFFFFF)**: clean white, the page.
- **On surface (#0A2540)**: navy on white.
- **Surface container (#EEF2F8)**: cool gray-blue, for cards, form sections, and the footer area.
- **Accent (#0038B8)**: Israeli flag blue, the primary attention-getter. CTAs, links, focus rings, error highlights are derived from it.
...
Heebo as a single workhorse. Inter sits in the stack as a Latin fallback for English-only surfaces and code fragments. The system uses weight (400 / 700 / 800) more aggressively than other systems to carry hierarchy without color or shape.
